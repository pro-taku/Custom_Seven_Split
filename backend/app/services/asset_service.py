...
class AssetService:
    ...
    # 종목 매수 기록
    def record_buy_stock(self, split_level: int, stock_code: str, price: int, count: int):
        """
        account_db에 stock_code가 (split_level)번 계좌에 추가됐음을 기록
        cach_flow_db에 매수금만큼 차감했음을 기록
        trade_check_db에 이번 매수 내용을 추가
        """
        account = AccountDB.get(self.db, split_level, stock_code)
        if account:
            AccountDB.update(self.db, split_level, stock_code, new_count=account.count + count)
        else:
            AccountDB.create(self.db, split_level, stock_code, price, count)
            
        amount = price * count
        # Assuming current deposit is handled externally or derived from total
        current_deposit = 0 
        CashFlow.create(self.db, deposit=current_deposit, flow_type="매수", amount=-amount)
        TradeCheckDB.create(self.db, stock_code, "BUY", price, count, status=1)

    # 종목 매도 기록
    def record_sell_stock(self, split_level: int, stock_code: str, price: int, count: int, trade_id: Optional[int] = None):
        """
        account_db에 stock_code가 (split_level)번 계좌에 삭제됐음을 기록
        cach_flow_db에 매수금만큼 수급됐음을 기록
        ...
        amount = price * count
        current_deposit = 0
        CashFlow.create(self.db, deposit=current_deposit, flow_type="매도", amount=amount)
        if trade_id:
    ...
    def record_cash_flow(self, flow_type: str, amount: int, current_deposit: int = 0):
        ...
        """
        CashFlow.create(self.db, deposit=current_deposit, flow_type=flow_type, amount=amount)
    ...
    async def asset_snapshot(self):
        ...
        for i in range(1, 8):
            ...
            for acc in accounts:
                if self.kis_client:
                    try:
                        current_price = await self.kis_client.get_current_price(acc.stock_code)
    ...
    async def get_virtual_account_asset(self, split_level: int):
        ...
        for acc in accounts:
            ...
            stock_dict[acc.stock_code]["투입자본"] += acc.price * acc.count
            stock_dict[acc.stock_code]["평가금액"] += current_price * acc.count
            stock_dict[acc.stock_code]["손익"] = stock_dict[acc.stock_code]["평가금액"] - stock_dict[acc.stock_code]["투입자본"]
